...
static struct page_table_t * get_page_table(
	...
	// if(!ret_pageTable) return ret_pageTable;
	return NULL;
	...
	if (mem_avail) {
		...
		for (int i = 0; i < NUM_PAGES ; i++ ) {
			if (_mem_stat[i].proc == 0) {
				...
				pageIndex = proc->seg_table->table[segIndex].pages->size - 1;

				proc->seg_table->table[segIndex].v_index = segIndex;
				proc->seg_table->table[segIndex].pages->table[pageIndex].v_index = get_second_lv(address);
				proc->seg_table->table[segIndex].pages->table[pageIndex].p_index = i;
				
				address += PAGE_SIZE;	// jump to the next page
				idx++;
				proc->seg_table->size++;
				if (idx == num_pages) {
					_mem_stat[i].next = -1;
					break;	// break if reaching number of pages need to be allocated.
				}
			}
